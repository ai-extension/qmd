...
describe("CLI Update Command - per-collection (-c) and embed (-e)", () => {
  ...
  async function setupTwoCollections() {
    ...
    const dirA = join(testDir, `update-c-alpha-${testCounter}`);
    const dirB = join(testDir, `update-c-beta-${testCounter}`);
    await mkdir(dirA, { recursive: true });
    await mkdir(dirB, { recursive: true });
    await writeFile(join(dirA, "a.md"), `# Alpha\nalpha content ${Date.now()}\n`);
    await writeFile(join(dirB, "b.md"), `# Beta\nbeta content ${Date.now()}\n`);
    const addA = await runQmd(["collection", "add", dirA, "--name", "alpha"], env);
    expect(addA.exitCode).toBe(0);
    const addB = await runQmd(["collection", "add", dirB, "--name", "beta"], env);
    expect(addB.exitCode).toBe(0);
    return env;
  }

  test("-c restricts the update to a single collection", async () => {
    const env = await setupTwoCollections();
    const { stdout, exitCode } = await runQmd(["update", "-c", "alpha"], env);
    expect(exitCode).toBe(0);
  ...
  test("-c accepts multiple collections", async () => {
    const env = await setupTwoCollections();
    const { stdout, exitCode } = await runQmd(["update", "-c", "alpha", "-c", "beta"], env);
    expect(exitCode).toBe(0);
    expect(stdout).toContain("Updating 2 collection(s)");
  });

  test("-c with an unknown collection exits with error", async () => {
    const env = await setupTwoCollections();
    const { stderr, exitCode } = await runQmd(["update", "-c", "does-not-exist"], env);
    expect(exitCode).toBe(1);
    expect(stderr).toContain("Collection not found");
  });

  test("without -c still updates all collections", async () => {
    const env = await setupTwoCollections();
