...
/**
 * Absolute external store override. When `CODEGRAPH_STORE` is set to an
 * absolute path, the index data directory is that path directly, instead of
 * `<projectRoot>/.codegraph`. This lets a host (qmd) keep each registered
 * graph's DB in a dedicated store folder OUTSIDE the indexed repo, so the repo
 * working tree stays clean and all graphs can be gathered in one place.
 *
 * Read live (like {@link codeGraphDirName}) so it's process-accurate/testable.
 * A relative/empty value is ignored — we never write the index to a
 * non-absolute external location.
 */
export function codeGraphStoreOverride(): string | null {
  const raw = process.env.CODEGRAPH_STORE?.trim();
 ...
 * Honors the `CODEGRAPH_STORE` absolute-path override (see above); otherwise
 * resolves to `<projectRoot>/<CODEGRAPH_DIR>`.
 */
export function getCodeGraphDir(projectRoot: string): string {
  const store = codeGraphStoreOverride();
