...
async function updateCollections(
  ...
  // Restrict to the requested collection(s) when -c is provided. An empty/omitted
  // filter keeps the original behavior of updating every collection. Names are
  // already validated by resolveCollectionFilter at the call site.
  if (opts?.collections && opts.collections.length > 0) {
    const wanted = new Set(opts.collections);
    collections = collections.filter(col => wanted.has(col.name));
  }
